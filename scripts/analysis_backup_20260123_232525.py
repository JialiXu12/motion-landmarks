...
# ==============================================================================
# SECTION 7: ADVANCED BIOMECHANICAL ANALYSIS
# ==============================================================================
...
def plot_anatomical_correlation_matrix(df):
    ...
    print(f"\n✓ Saved correlation matrix: {save_path}")
...
def compare_reference_frames(df):
    ...
    return {
        ...
        'correlation': correlation if len(paired_df) > 0 else None,
        'p_value': p_value if len(paired_df) > 0 else None
    }

# ==============================================================================
# BREAST LANDMARK MOTION ANALYSIS
# Analysis of breast tissue landmark displacement from prone to supine position
# ==============================================================================
...
def perform_group_analysis(data_df, dv_col, group_col):
    ...
    if num_groups < 3:
        print(f"! Warning: Only {num_groups} groups found. ANOVA/Kruskal-Wallis requires 3+ groups.")
        return
...
def perform_two_group_analysis(data_df, dv_col, group_col):
    ...
    if len(data_A) < 5 or len(data_B) < 5:
        print(f"! Warning: Small sample sizes (N_A={len(data_A)}, N_B={len(data_B)}) may affect test reliability.")
...
def perform_repeated_measures_analysis(df_input, subject_id_col, dv_cols):
    ...
    try:
        ...
        print("eta² = partial eta squared (effect size)")
...
# ==============================================================================
# SECTION 3: CORRELATION AND REGRESSION ANALYSIS
# ==============================================================================
...
def plot_sagittal_dual_axes(df_ave, color_by='breast', vl_id=None):
    ...
    plt.show()
    plt.close(fig)
    print(f"[OK] Completed dual sagittal plot with '{color_by}' coloring")


# ==============================================================================
# SECTION 5: VISUALIZATION FUNCTIONS - NIPPLE REFERENCE FRAME
# ==============================================================================
...
def analyse_clock_position_rotation(df_ave, base_left=None, base_right=None, vec_left=None, vec_right=None, save_dir=None):
    ...
    for df_subset, breast_side in [(df_left, 'Left'), (df_right, 'Right')]:
        ...
        if p_val < 0.05:
            ...
            print(f"[OK] Significant {direction} rotation detected!")
        else:
    ...
    for df_subset, breast_side in [(df_left, 'Left'), (df_right, 'Right')]:
        ...
        # Use annotate for arrow in polar coordinates (arrow() doesn't work correctly)
        ax2.annotate('',
    ...
    if len(df_left) > 0 and len(df_right) > 0:
        ...
        # Plot individual points (lighter - same as right subplot of individual breast plot)
        ...
        ax_left.plot([mean_theta_prone_r, mean_theta_supine_r],
                    ...
                    'black', linewidth=2, zorder=10)
        # Use annotate for arrow in polar coordinates
        ax_left.annotate('',
        ...
        ax_left.set_ylabel('Distance from Nipple (mm)', labelpad=30)
        ...
        ax_right.set_ylabel('Distance from Nipple (mm)', labelpad=30)
...
def compare_reference_frames(df):
    ...
    # ...existing code...
    # [Full function body would go here - abbreviated for space]

    return {
        'sternum_mean': 0,  # Placeholder
        'nipple_mean': 0,
        'difference': 0,
        'correlation': None,
        'p_value': None
...
if __name__ == "__main__":
    ...
    if 'left' in clock_summary:
        ...
        if left_stats['p_value'] < 0.05:
            print(f"\n  [OK] HYPOTHESIS CONFIRMED: Significant {direction} rotation detected for left-sided landmarks")
    ...
    if 'right' in clock_summary:
        ...
        if right_stats['p_value'] < 0.05:
            print(f"\n  [OK] HYPOTHESIS CONFIRMED: Significant {direction} rotation detected for right-sided landmarks")
        else:
            print(f"\n  ✗ HYPOTHESIS NOT CONFIRMED: No significant rotation detected")
    ...
    plot_3panel_anatomical_views(df_ave, save_path=None)

    # Correlation matrix analysis
    plot_anatomical_correlation_matrix(df_ave)

    # Reference frame comparison analysis
    compare_reference_frames(df_ave)
