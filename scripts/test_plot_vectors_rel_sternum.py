"""
Test file for plot_vectors_rel_sternum function
...
from analysis import read_data, plot_vectors_rel_sternum
...
def test_plot_vectors_rel_sternum():
    """Test plot_vectors_rel_sternum with different coloring options"""

    print("=" * 80)
    print("TESTING plot_vectors_rel_sternum FUNCTION")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("TEST 4: Filter by specific subject (VL_ID=81) with DTS coloring")
    print("=" * 80)

    # Find a valid VL_ID from the data
    unique_vl_ids = df_ave['VL_ID'].unique()
    if len(unique_vl_ids) > 0:
        test_vl_id = unique_vl_ids[0]
        print(f"Testing with VL_ID: {test_vl_id}")
        try:
            plot_vectors_rel_sternum(df_ave, color_by='dts', vl_id=test_vl_id)
    ...
    print("=" * 80)
    print("\nFunction plot_vectors_rel_sternum is working correctly with:")
...
if __name__ == "__main__":
    success = test_plot_vectors_rel_sternum()
