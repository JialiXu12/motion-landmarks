...
from traits.api import HasTraits, Instance, on_trait_change, Button, Range, Enum, Bool
...
def plot_meshes(fig,labels,meshes,color):
    for mesh_idx, mesh in enumerate(meshes):
        plot_mesh(fig, mesh, labels[mesh_idx], color=color, line_colour=(0,1,0), opacity=0.3)
...
class Figure:
    ...
    def plot_edges(self, label, X, color=(1,0,0), opacity=1., line_width=1):
        ...
        view = mlab.view()
        roll = mlab.roll()
        self.plots[label] = mlab.pipeline.surface(edges, opacity=opacity, line_width=line_width, color=(1,0,0),figure=self.figure) #, opacity=opacity,  color=color, figure=self.figure)
        mlab.view(*view)
        mlab.roll(roll)
        self.figure.scene.disable_render = False
        mlab_obj = self.plots.get(label)
...
class Visualization(HasTraits):
    ...
    volunteer_dropdown = Enum(["VL00046","VL00047","VL00048"])
    label_checkbox = Bool('Select labels')

    @on_trait_change('reset_views_btn')
    def reset_views(self):
        if hasattr(self, 'prone_default_view_point1'):
            self.prone_ipw.ipw.point1 = self.prone_default_view_point1
            self.prone_ipw.ipw.point2 = self.prone_default_view_point2
            self.prone_ipw.ipw.origin = self.prone_default_view_origin
            self.prone_ipw.ipw.update_placement()

        if hasattr(self, 'supine_default_view_point1'):
            self.supine_ipw.ipw.point1 = self.supine_default_view_point1
            self.supine_ipw.ipw.point2 = self.supine_default_view_point2
            self.supine_ipw.ipw.origin = self.supine_default_view_origin
            self.supine_ipw.ipw.update_placement()
    ...
    def add_plots1(self,scene):
        ...
        nii_path = os.path.join(mechanics_dir,'prone_mri_aligned_with_model.nii.gz')
        nii_mask_path = os.path.join(mechanics_dir,'prone_mask.nii.gz')
        if os.path.exists(nii_path):
            ...
            self.prone_ipw = mlab.pipeline.image_plane_widget(
                                outline,
            ...
            self.prone_default_view_point1 = self.prone_ipw.ipw.point1
            self.prone_default_view_point2 = self.prone_ipw.ipw.point2
            self.prone_default_view_origin = self.prone_ipw.ipw.origin
            self.prone_ipw.ipw.middle_button_action = 0
        ...
        def select_point(obj, evt):
            ...
            p0 = self.prone_ipw.ipw.point1
            p1 = self.prone_ipw.ipw.point2
            p3 = self.prone_ipw.ipw.origin
            ...
            if self.label_checkbox:
                ...
                self.supine_ipw.ipw.point1 = p0+u
                self.supine_ipw.ipw.point2 = p1+u
                self.supine_ipw.ipw.origin = p3+u
                self.supine_ipw.ipw.update_placement()
            else:
            #import ipdb; ipdb.set_trace()
        ...
        self.prone_ipw.ipw.add_observer('EndInteractionEvent', select_point)
    ...
    def add_plots2(self,scene):
        ...
        nii_path = os.path.join(mechanics_dir,'warped_prone_mri_aligned_with_model' + save_prefix + '.nii.gz')
        if os.path.exists(nii_path):
            image= nii.load(nii_path)
            self.pixdim = image.header['pixdim'][1:4]
            self.img_shape = image.shape
            ...
            self.supine_ipw = mlab.pipeline.image_plane_widget(
                                outline,
                                plane_orientation='z_axes',slice_index=int(0.5 * image.header.get_n_slices()),colormap='black-white')
            self.supine_default_view_point1 = self.supine_ipw.ipw.point1
            self.supine_default_view_point2 = self.supine_ipw.ipw.point2
            self.supine_default_view_origin = self.supine_ipw.ipw.origin
            self.supine_ipw.ipw.middle_button_action = 0
        ...
        def select_point(obj, evt):
            ...
            p0 = self.supine_ipw.ipw.point1
            p1 = self.supine_ipw.ipw.point2
            p3 = self.supine_ipw.ipw.origin
            u = points_to_plane(p0,p1,p3,transformed_pt/self.pixdim)

            self.prone_ipw.ipw.point1 = p0+u
            self.prone_ipw.ipw.point2 = p1+u
            self.prone_ipw.ipw.origin = p3+u
            self.prone_ipw.ipw.update_placement()
        ...
        self.supine_ipw.ipw.add_observer('EndInteractionEvent', select_point)
...
#        self.scene1.mlab.test_points3d()
    ...
    view = View(HSplit(
                  Group(
                      'volunteer_dropdown','reset_views_btn','_','label_checkbox', 'ptSize','_','clear_points_btn','clear_labels_btn','_', 'generate_landmarks_btn','clear_landmarks_btn',
                      show_labels=False,
                  ),
                  Group(
                      Item('scene1',
