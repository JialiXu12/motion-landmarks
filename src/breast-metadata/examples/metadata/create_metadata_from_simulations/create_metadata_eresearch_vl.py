...
def path_if_exists_cl(data_dir, process_dir, uid):
    """Returns the path to data if it exists, else returns nan.
    ...
    if os.path.exists(os.path.join(data_dir, process_dir, uid)):
        return os.path.join(process_dir, uid)
    else:
        return np.nan
...
if __name__ == '__main__':
    ...
    prefix = ''
    if dataset_type == 'volunteer':
        num_participants = 115
        prefix = 'VL'
        ...
        # metadata.
        data_dir = 'X:\\projects\\volunteer_camri\\metadata'
        ...
        num_participants = 211
        prefix = 'CL'
        # Define location to export newly created metatdata.
        metadata_filename = 'metadata_prone_to_supine_t1_2017_04_18.csv'
        # Specify location of data and directories to walk through for extracting
        # metadata.
        data_dir = 'X:\\projects\\clinical_adhb\\metadata'
        image_dir = '..\\data\\images'
        segmentation_dir = '..\\processed\\prone_to_supine_t1\\2017_04_18\\segmented_data'
        mesh_dir = '..\\processed\\prone_to_supine_t1\\2017_04_18\\volunteer_meshes'
        mechanics_dir = '..\\processed\\prone_to_supine_t1\\2017_04_18\\mechanics_meshes'
        landmarks_dir = '..\\processed\\landmarks'
    ...
    for participant_idx in range(num_participants):
        participant_ids.append("{0}{1:05d}".format(prefix, participant_idx))
    ...
    if dataset_type == 'volunteer':
        ...
        for participant_idx, uid in enumerate(df['participant_id']):
            ...
            df.at[participant_idx, 'mri_t1_prone_arms_down_image_path'] = path_if_exists(
                data_dir, image_dir, "{0:05d}".format(participant_idx), 'MRI_T1_prone')
            # T2.
            df.at[participant_idx, 'mri_t2_prone_arms_down_image_path'] = path_if_exists(
                data_dir, image_dir, "{0:05d}".format(participant_idx), 'MRI_T2_prone')

            # Supine.
            # T1.
            df.at[participant_idx, 'mri_t1_supine_arms_down_image_path'] = path_if_exists(
                data_dir, image_dir, "{0:05d}".format(participant_idx), 'MRI_T1_supine')
            # T2.
            df.at[participant_idx, 'mri_t2_supine_arms_down_image_path'] = path_if_exists(
                data_dir, image_dir, "{0:05d}".format(participant_idx), 'MRI_T2_supine')
    elif dataset_type == 'clinical':
        # Add values to the empty columns.
        for participant_idx, uid in enumerate(df['participant_id']):
            # Prone.
            # T1.
            df.at[participant_idx, 'mri_t1_prone_arms_down_image_path'] = path_if_exists_cl(
                data_dir, image_dir, "{0:05d}".format(participant_idx))
