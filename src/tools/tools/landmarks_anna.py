...
class Metadata(object):
    ...
    def load_metadata_files(self,dicom_path):
        ...
        try :
            dicom_image = dicom.read_file(dicom_file)
        ...
        for dicom_file in os.listdir(dicom_path):
            try:
                dicom_slice = dicom.read_file(os.path.join(dicom_path,dicom_file))
                valid_dicom = True
            except dicom.filereader.InvalidDicomError:
                valid_dicom = False
            if valid_dicom:
        ...
        if len(slices_position)>0:
            ...
            self.image_position = np.array([slices_position.max(0)[1],slices_position.min(0)[0],slices_position.min(0)[2]])
        ...
        if os.path.exists(file_name):
             ...
             if len(sternum_position):
                 ...
                 self.jugular_notch = np.array([pt['y'] - self.image_position[0],
                                                self.pixel_spacing[1] * self.image_shape[1] -pt['x'] + self.image_position[1],
        ...
        if os.path.exists(nipple_path) and len(os.listdir(nipple_path))==2:
            ...
            if len(right_nipple_position):
                ...
                self.right_nipple = np.array([pt['y'] - self.image_position[0],
                    -pt['x'] + self.image_position[1] + self.pixel_spacing[1] * self.image_shape[1],
                    pt['z'] - self.image_position[2]])
...
class Landmarks:
    ...
    def find_quadrants(self, metadata):
        # compute the quadrant were the landmarks are located
        # given the nipple position and sternum position
        # the nipple and sternum position are read from metadata object
        # the metadata disctionary should have the same size and volunteers ids
        # as the landmarks object
                ...
                if not skip:
                    ...
                    for landmark in self.landmarks[vl_id]:
                        if landmark[1] < metadata[vl_id].jugular_notch[1]:  # LHS
                            if landmark[1] < metadata[vl_id].left_nipple[1]:  # UO or LO
                                ...
                                if landmark[2] > metadata[vl_id].right_nipple[2]:  # UI
                                    ...
                                    self.quadrants[vl_id].append('LB_LI')

                        else:  # RHS
                            if landmark[1] < metadata[vl_id].right_nipple[1]:  # UI or LI
                                if landmark[2] > metadata[vl_id].right_nipple[2]:  # UI

                                    self.quadrants[vl_id].append('RB_UI')
                                else:  # LI

                                    self.quadrants[vl_id].append('RB_LI')
                            else:  # UO or LO
                                if landmark[2] > metadata[vl_id].right_nipple[2]:  # UO
                                    ...
                                    self.quadrants[vl_id].append('RB_LO')




    def getModellandmarks(self, metadata):
        #Transform points in RAF coordinates system (medical imaging)
        # to  model coordinates system
        ...
        for vl_id in self.landmarks:
            ...
            print ('Subject {0} transform landmarls to model reference system'.format(vl_str))
    ...
    def getRAFCoordinates(self, metadata):
        ...
        for vl_id in self.landmarks:
            ...
            vl_str = 'VL{0:05d}'.format(vl_id)
            print ('Subject {0} transform landmarls to RAF reference system'.format(vl_str))
        ...
        for vl_id in proneLd.vl_ids:
            ...
            if vl_id in proneLd.landmarks:
                ...
                for indx in range(len(proneLd.landmarks[vl_id])):
                    ...
                    data = {"{0}_point".format(ld_type_1): {"time": 0.0, "point": {"x": landmark[0], "y": landmark[1], "z": landmark[2]}},  "user":user_id , "time": 0.0, "type": ld_type, "id": indx+1, "subject": vl_str}
                    if supinePoints:
                        landmark = supineLd.landmarks[vl_id][indx]
                        data["{0}_point".format(ld_type_2)] = {"time": 0.0, "point": {"x": landmark[0], "y": landmark[1], "z": landmark[2]}}
...
def corresponding_landmarks_between_registars(registrar_a_prone, registrar_b_prone, registrar_a_supine, registrar_b_supine):
